...
# === 单线程核心翻译函数 ===
def translate_text(text: str, is_caption: bool) -> str:
    ...
    1. 保持学术严谨性。
    2. 公式必须用 $...$ 或 $$...$$ 包裹。
    3. 如果原文公式有破损，请结合上下文修复为 LaTeX 格式。
    4. 直接输出译文，不要加任何前缀或解释。
    """
    if is_caption: sys_prompt += " (这是图注，请保留 Figure 编号)"

    # 单线程模式下，我们也可以稍微重试一下，防止网络抖动
    for attempt in range(2):
        ...
        try:
            ...
            # 简单的防乱码检查
            if "翻译失败" not in result:
                return result
        except Exception as e:
            print(f"Error: {e}")
            time.sleep(1) # 歇一秒再试
    
    return text # 实在不行返回原文，不报错

def capture_image_between_blocks(page, prev_bottom, current_top):
    if current_top - prev_bottom < 15: return None # 稍微放宽
    rect = fitz.Rect(50, prev_bottom + 2, page.rect.width - 50, current_top - 2)
    try:
        pix = page.get_pixmap(matrix=fitz.Matrix(3, 3), clip=rect, alpha=False)
        img = Image.open(io.BytesIO(pix.tobytes("png")))
        return img if img.size[1] >= 15 else None
    except: return None

# === 单线程处理逻辑 ===
def process_page_elements(elements):
    # 彻底移除 ThreadPoolExecutor，改用 for 循环
    # 虽然慢，但是最稳，不会崩，不会乱
    for el in elements:
        if el['type'] in ['text', 'caption']:
            # 逐个翻译
            el['content'] = translate_text(el['content'], el['type'] == 'caption')
    return elements
...
def parse_page(page):
    ...
    # 调用单线程处理
    return process_page_elements(raw_elements)
...
def generate_html(doc, start, end, mode="pure", filename="Document", font_size=16, line_height=1.6, img_width=50):
    ...
    for i, page_num in enumerate(range(start, end + 1)):
        status_text.text(f"正在处理第 {page_num} 页 (单线程模式，请耐心等待)...")
...
def html_to_pdf_with_chrome(html_content, output_pdf_path):
    ...
    cmd = [
        ...
        "--virtual-time-budget=10000", # 单线程可能需要更久渲染
...
# --- 5. 界面逻辑 ---
st.title("🔬 光学室学术论文翻译 (V38 单线程·求稳版)")
...
if uploaded_file:
    ...
    if app_mode == "👁️ 实时预览":
        ...
        if st.session_state.get('run_preview'):
             with st.spinner("🚀 正在单线程逐段翻译 (速度较慢，请稍候)..."):
        ...
        if st.button(f"🚀 生成 PDF", type="primary"):
            if not VALID_KEYS:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp_pdf:
                    ...
                    if ok:
                        bar.progress(100)
                        status.success("✅ 完成！稳得一批！")
                        fname = "Translation_V38_SingleThread.pdf"
